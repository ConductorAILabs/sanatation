...
class LTX2RotaryPosEmbed1d(nn.Module):
    ...
    def forward(
        ...
        # MPS doesn't support float64; downcast.
        _dev_type = device.type if isinstance(device, torch.device) else str(device).split(":")[0]
        freqs_dtype = torch.float64 if (self.double_precision and _dev_type != "mps") else torch.float32
