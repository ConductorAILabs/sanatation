...
def causal_conv1d(
    ...
    **kwargs,
    ...
    """
    # Pure-PyTorch fallback for MPS/CPU (no Triton, no CUDA, no fla custom kernels).
    import os as _os, torch.nn.functional as _F
    _force_pt = (
        x.device.type in ("mps", "cpu")
        or _os.environ.get("SANA_WM_FORCE_PURE_PYTORCH", "0") == "1"
        or _os.environ.get("FLA_USE_PURE_PYTORCH", "0") == "1"
    )
    if _force_pt and cu_seqlens is None and cp_context is None and initial_state is None:
        # x: (B, T, D), weight: (D, W) — depthwise causal conv.
        # Canon op order (matches Triton kernel + cuda/ops.py): conv → +bias → silu → +residual.
        B, T, D = x.shape
        W = weight.shape[-1]
        x_t = x.transpose(1, 2)  # (B, D, T)
        x_pad = _F.pad(x_t, (W - 1, 0))
        w_conv = weight.unsqueeze(1)  # (D, 1, W) depthwise
        y = _F.conv1d(x_pad, w_conv, bias=bias, groups=D)
